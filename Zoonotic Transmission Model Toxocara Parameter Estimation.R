...
run_country_model <- function(country_name, Input_parameters, Input_demographics) {
  # Progress message
  print(paste("Processing", country_name))
  
  # Extract parameters for specific country
  ...
  # Calculate transmission rates and CIs
  calculate_transmission_params <- function(
    estimated_infections_low,
    estimated_infections_mean, 
    estimated_infections_high,
    total_population, 
    scaling_factors, 
    animal1_pop_2021,
    animal1_pop_2022,
    ...
    animal1_prevalence, 
    animal2_prevalence, 
    initial_recovered_proportion) {
    
    # Calculate stable populations
    calculate_stable_population <- function(pop_2021, pop_2022, pop_2023) {
      populations <- c(pop_2021, pop_2022, pop_2023)
      med <- median(populations)
      mad <- median(abs(populations - med))
      valid_pops <- populations[abs(populations - med) <= 2 * mad]
      return(mean(valid_pops))
    }
    
    # Get stable populations for calculations
    animal1_stable_pop <- calculate_stable_population(animal1_pop_2021, animal1_pop_2022, animal1_pop_2023)
    animal2_stable_pop <- calculate_stable_population(animal2_pop_2021, animal2_pop_2022, animal2_pop_2023)
    
    calculate_infections <- function(beta1, beta2) {
      total_infections <- 0
      susceptible <- total_population * (1 - initial_recovered_proportion)
      
      for (i in 1:length(scaling_factors)) {
        beta_age_group1 <- beta1 * scaling_factors[i]
        beta_age_group2 <- beta2 * scaling_factors[i]
        
        infections1 <- beta_age_group1 * animal1_prevalence * susceptible * animal1_stable_pop / total_population
        infections2 <- beta_age_group2 * animal2_prevalence * susceptible * animal2_stable_pop / total_population
        
        total_infections <- total_infections + (infections1 + infections2)
      }
      
      return(total_infections * 12)
    }
    
    optimize_for_infections <- function(target_infections) {
      objective_function <- function(params) {
        beta1 <- params[1]
        beta2 <- params[2]
        predicted <- calculate_infections(beta1, beta2)
        return((predicted - target_infections)^2)
      }
      ...
      return(c(result$p1[which.min(result$value)], 
               result$p2[which.min(result$value)]))
    }
    
    # Find fits for each infection level
    mean_fit <- optimize_for_infections(estimated_infections_mean)
    low_fit <- optimize_for_infections(estimated_infections_low)
    high_fit <- optimize_for_infections(estimated_infections_high)
    
    # Order results properly for CIs
    beta1_lower <- min(low_fit[1], mean_fit[1], high_fit[1])
    beta1_upper <- max(low_fit[1], mean_fit[1], high_fit[1])
    beta2_lower <- min(low_fit[2], mean_fit[2], high_fit[2])
    beta2_upper <- max(low_fit[2], mean_fit[2], high_fit[2])
    
    return(list(
      beta1 = mean_fit[1],
      beta2 = mean_fit[2],
      beta1_ci_lower = beta1_lower,
      beta1_ci_upper = beta1_upper,
      beta2_ci_lower = beta2_lower,
      beta2_ci_upper = beta2_upper
    ))
  }  
  
  # Call function with all three infection estimates
  transmission_params <- calculate_transmission_params(
    estimated_infections_low = country_params$estimated_infections_lowCI,
    estimated_infections_mean = country_params$estimated_infections,
    estimated_infections_high = country_params$estimated_infections_uppCI,
    ...
    animal2_pop_2022 = country_params$animal2_pop_2022,
    animal2_pop_2023 = country_params$animal2_pop_2023,
    animal1_prevalence = animal1_prevalence,
    animal2_prevalence = animal2_prevalence,
    initial_recovered_proportion = initial_recovered_proportion
  )
  ...
  for(iteration in 1:100) {
    ...
    populations <- data.frame(
      ...
      Total_New_Exposures = numeric(120),  # Changed from Total_New_Infections
      Symptomatic_Cases = numeric(120),    # New column
      ...
      Cumulative_Exposures = numeric(120)  # Changed from Cumulative_Unique_Infected_Humans
    )
...
all_results <- lapply(Input_parameters$Country, function(country) {
  ...
  tryCatch({
    result <- run_country_model(country, Input_parameters, Input_demographics)
